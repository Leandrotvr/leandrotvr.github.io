...
/* ===== LOOP ===== */
function loop(ts){ if(!running) return; if(!last) last=ts; const dt=ts-last; last=ts; update(dt); draw(); if(running) requestAnimationFrame(loop); }

/* ===== UPDATE ===== */
function update(dt){
  timeLeft-=dt; if(timeLeft<=0){ timeLeft=0; running=false; finalizar(); }
...
function syncGoalOverlays(){
  ...
  const gateW = 40*scaleX, gateH = 40*scaleY;
...
function resetAll(){
  ...
  last=0; running=false; taunt=null; BALL_SPEED=BALL_SPEED_BASE;
  ball=resetBall();
...
function drawCaricaturePaddle(p,face,side){
  ...
  ctx.translate(cx,cy); ctx.rotate(Math.sin(perf()*0.002+cx)*0.02);
  ...
  ctx.globalAlpha=0.9; ctx.fillStyle="#fff"; ctx.font="16px system-ui"; ctx.textAlign="center"; ctx.fillText(face.emoji,0,-ry-6);
  ctx.restore();
}

function drawBall(b,skin){ ctx.save(); ctx.translate(b.x,b.y);
  switch(skin){
...
function drawFieldTheme(theme){
  const [bg,border,line]=theme; const g=ctx.createLinearGradient(0,0,0,H);
...
function drawTaunt(){
  ctx.save(); const k=(Date.now()-taunt.t0)/taunt.ttl, e=Math.max(0,1-k);
  let x=taunt.x,y=taunt.y,s=1,r=0,skew=0;
  switch(taunt.anim){case'shake':x+=Math.sin(perf()*0.06)*6;break;case'wiggle':r=Math.sin(perf()*0.01)*0.25;break;case'bounce':y-=Math.abs(Math.sin(perf()*0.008))*14;break;case'pop':s=1+0.4*e;break;case'spin':r=perf()*0.01;break;case'wobble':r=Math.sin(perf()*0.02)*0.4;s=1+Math.sin(perf()*0.04)*0.05;break;case'slideL':x-=e*40;break;case'slideR':x+=e*40;break;case'slideU':y-=e*40;break;case'slideD':y+=e*40;break;case'jelly':s=1+Math.sin(perf()*0.05)*0.2;break;case'pulse':s=1+Math.sin(perf()*0.08)*0.1;break;case'skew':skew=Math.sin(perf()*0.05)*0.3;break;case'flipX':r=Math.PI*(e);break;case'flipY':r=Math.PI*(e);break;case'swing':r=Math.sin(perf()*0.02)*0.6;break;case'breath':s=1+Math.sin(perf()*0.02)*0.15;break;case'rainbow':;break;case'spark':;break;case'zoom':s=1+0.8*e;break;}
  ...
  const text=taunt.texto,pad=10; ctx.font='bold 18px system-ui'; ctx.textAlign='center';
  const w=ctx.measureText(text).width+pad*2,h=30; ctx.globalAlpha=0.85*e+0.15;
  ctx.fillStyle='#000c'; roundRect(ctx,-w/2,-h-6,w,h,10,true,false);
  ctx.fillStyle=color; ctx.fillText(text,0,-h/2-6+6); ctx.restore();
}
...
function finalizar(){ const g=ganador(); hud.status.textContent=`Fin • Ganador: ${g.etiqueta} (menos goles: ${g.goles})`; ctx.save(); ctx.fillStyle='#000a'; ctx.fillRect(0,0,W,H); ctx.fillStyle='#eaf2ff'; ctx.textAlign='center'; ctx.font='bold 34px system-ui'; ctx.fillText('¡Fin del juego!',W/2,H/2-30); ctx.font='600 24px system-ui'; ctx.fillText(`Ganador: ${g.etiqueta} (menos goles: ${g.goles})`,W/2,H/2+10); ctx.font='16px system-ui'; ctx.fillText('⟲ Reinicia para jugar otra ronda',W/2,H/2+40); ctx.restore(); }
function perf(){ return performance.now(); }
function randomAngle(){ for(;;){ const a=Math.random()*Math.PI*2,deg=a*180/Math.PI,m=(deg+360)%360; const near=[0,90,180,270].some(ax=>Math.abs(((m-ax+540)%360)-180)<10); if(!near) return a; } }
function clamp(v,min,max){ return Math.max(min,Math.min(max,v)); }
function rnd(a,b){ return Math.floor(Math.random()*(b-a+1))+a; }
function rand(arr){ return arr[(Math.random()*arr.length)|0]; }
function shade(hex,amt){ const c=parseInt(hex.slice(1),16); let r=(c>>16)+Math.round(255*amt), g=((c>>8)&255)+Math.round(255*amt), b=(c&255)+Math.round(255*amt); r=Math.max(0,Math.min(255,r)); g=Math.max(0,Math.min(255,g)); b=Math.max(0,Math.min(255,b)); return '#'+((1<<24)+(r<<16)+(g<<8)+b).toString(16).slice(1); }
...
function ellipse(x,y,rx,ry,fill){ ctx.beginPath(); for(let i=0;i<Math.PI*2;i+=Math.PI/24){ const px=x+rx*Math.cos(i), py=y+ry*Math.sin(i); i===0?ctx.moveTo(px,py):ctx.lineTo(px,py);} ctx.closePath(); if(fill) ctx.fill(); else ctx.stroke(); }
function fillCircle(c,r){ ctx.fillStyle=c; ctx.beginPath(); ctx.arc(0,0,r,0,Math.PI*2); ctx.fill(); }
...
function yinYang(r){ fillCircle('#fff',r); ctx.fillStyle='#000'; ctx.beginPath(); ctx.arc(0,0,r,Math.PI/2,-Math.PI/2); ctx.fill(); fillCircle('#000',r/2); ctx.translate(0,-r/2); fillCircle('#fff',r/6); ctx.translate(0,r); fillCircle('#000',r/6); }
